...
def get_indexes_by_names(all_names, selected_names):
    indexes = []
    for name in selected_names:
        if name in all_names:
            indexes.append(all_names.index(name))
        else:
            for suffix in "XYZ":
                name_suffix = name + "-" + suffix
                if name_suffix in all_names:
                    indexes.append(all_names.index(name_suffix))
                else:
                    print("Feature {} not found!".format(name_suffix))
    return indexes
